...
const compute2 = ComputeBudgetProgram.setComputeUnitLimit({
    units: 600_000
})
...
export class Amm {
    ...
    async createFundTx(pubkeys, jitoTipLamports, blockhash) {
        ...
        const accounts = [{
            ...
            {
                pubkey: pubkeys[0].publicKey,
                isSigner: false,
                isWritable: true
            },
            {
                pubkey: pubkeys[1].publicKey,
                isSigner: false,
                isWritable: true
            },
            {
                pubkey: pubkeys[2].publicKey,
                isSigner: false,
                isWritable: true
            },
            {
                pubkey: pubkeys[3].publicKey,
                isSigner: false,
                isWritable: true
            },
            {
                pubkey: selectedJitoAccount,
                isSigner: false,
                isWritable: true
            },
            {
    ...
    async swapMakers(direction, mint, signer, blockhash) {
        ...
        let instructionData
        if (direction === "buy") {
            instructionData = Buffer.concat([Buffer.from([2]), Buffer.from([1]), Buffer.from(parsedSwapIx.data, 'base64')])
        }
        if (direction === "sell") {
            instructionData = Buffer.concat([Buffer.from([2]), Buffer.from([0]), Buffer.from(parsedSwapIx.data, 'base64')])
        }
        let swapIx = new TransactionInstruction({
            keys: [...parsedSwapIx.keys.map(acc => ({
    ...
    async makers(mint, totalMakersRequired, options = {}) {
        ...
        let testSent = null
        try {
            testSent = await this.connection.sendRawTransaction(testVtx.serialize(), {
                skipPreflight: false,
                preflightCommitment: "finalized"
            })
        } catch (E) {
            console.log("Blockhash test failed, try a different commitment or RPC. Your bundles will NOT land right now no matter what size Jito tip.")
        }
        if (testSent) {
            console.log("Blockhash test OK. Your bundles should land now.")
        }
        ...
        while (stats.makersCompleted < totalMakersRequired) {
            try {
                ...
                console.error("Error in makers:", err)
            }
        }
        stats['finished'] = true
        return stats
